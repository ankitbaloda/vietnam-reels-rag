...
def build_chunks(source_dir: Path, max_tokens: int, overlap_tokens: int) -> List[DocChunk]:
    ...
    for file in iter_source_files(source_dir):
        ...
        parts = hierarchical_chunks(content, max_tokens_per_chunk=max_tokens, overlap_tokens=overlap_tokens)
...
def main():
    parser = argparse.ArgumentParser(description="Build hierarchical embeddings index into Qdrant")
    parser.add_argument("--source-dir", default=os.getenv("SOURCE_DIR", "data/source"))
    parser.add_argument("--collection", default=os.getenv("QDRANT_COLLECTION", "flowise_reels"))
    parser.add_argument("--embeddings-model", default=os.getenv("EMBEDDINGS_MODEL", "text-embedding-3-large"))
    parser.add_argument("--qdrant-url", default=os.getenv("QDRANT_URL", "http://localhost:6333"))
    parser.add_argument("--qdrant-api-key", default=os.getenv("QDRANT_API_KEY"))
    parser.add_argument("--max-tokens-per-chunk", type=int, default=int(os.getenv("MAX_TOKENS_PER_CHUNK", "800")))
    parser.add_argument("--overlap-tokens", type=int, default=int(os.getenv("OVERLAP_TOKENS", "100")))
    ...
    # Build chunks
    all_chunks = build_chunks(source_dir, max_tokens=args.max_tokens_per_chunk, overlap_tokens=args.overlap_tokens)
    if not all_chunks:
